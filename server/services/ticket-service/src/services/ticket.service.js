...
export const updateTicketStatus = async (req, res) => {
  try {
    const { ticketId, userId, groupId } = req.body;
    const { event_status } = req.body;

    // Validate required parameters
    if (!ticketId || !userId || !groupId) {
      return res.status(400).json({ 
        message: "Missing required parameters",
        required: ["ticketId", "userId", "groupId"]
      });
    }

    if (!['pending', 'confirmed', 'cancelled'].includes(event_status)) {
      return res.status(400).json({ message: "Invalid event status" });
    }

    const updatedTicket = await Ticket.findOneAndUpdate(
      { _id: ticketId, userId: userId, groupId: groupId },
      {
        event_status,
        updated_by: userId,
        updated_at: new Date()
